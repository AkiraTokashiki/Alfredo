...
- **Control optional integrations.** MCP Streamable HTTP and LLM connectors expose local data to the client or configured provider. Bind HTTP to an appropriate interface, add authentication at the deployment boundary, use TLS when crossing hosts, and limit API-key permissions. Offline mode itself makes no API-key request, but it does not secure an exposed database.
